...
st.set_page_config(page_title="XAUUSD Signal Pro", page_icon="🥇", layout="wide")
    ...
    try:
        ...
        for ev in eventos:
            ...
            try:
                ...
                if diff <= 30:
                    return True, ev.get("title","Noticia alto impacto")
            except: continue
        return False, ""
    except: return False, ""
...
def detectar_vela_rejeicao(df, trend):
    ...
    if trend == "BUY":
        ...
        # Bullish Pin Bar: sombra inferior longa
        pin_bar = sombra_inf > amplitude_u * 0.6 and float(u["Close"]) > float(u["Open"])
        # Bullish Engulfing: vela actual engole anterior
...
def analise_m5(trend):
    try:
        ...
        vol_med = float(d["vol_medio"].iloc[-1]) if float(d["vol_medio"].iloc[-1])>0 else 1

        # Retracao: preco perto da EMA20 M5
        ...
        # Rompimento confirmado: vela anterior fecha alem da maxima/minima da vela de rejeicao
        u = d.iloc[-1]; prev = d.iloc[-2]
        if trend == "BUY":
            rompimento = float(u["Close"]) > float(prev["High"])
        else:
            rompimento = float(u["Close"]) < float(prev["Low"])

        # Volume
        vol_ok = vol > vol_med

        # SL: ultimo fundo/topo relevante M5
        d2=d.copy()
...
def filtro_sr(trend, preco, atr):
    try:
        d = obter_dados("1h","10d")
        d = adicionar_indicadores(d)
        highs = d["High"].tail(50)
        lows = d["Low"].tail(50)
        resistencia = float(highs.max())
        suporte = float(lows.min())
        if trend == "BUY":
...
# ==================== MOTOR PRINCIPAL ====================
def analisar(ignorar_sessao=False):
    try:
        s = sessao_activa()
        ...
        # Sinal so com score >= 70 e sessao activa e todos obrigatorios
...
def enviar_email(r):
    try:
        ...
        corpo = (
            r["emoji"]+" XAUUSD "+r["trend"]+" — "+r["confianca"]+chr(10)+
            "================================"+chr(10)+
            "Par:        XAUUSD"+chr(10)+
            "Direccao:   "+r["trend"]+chr(10)+
            "Entrada:    "+str(r["preco"])+chr(10)+
            "Stop Loss:  "+str(r["sl"])+chr(10)+
            "Take Profit:"+str(r["tp"])+chr(10)+
            "Risk Reward:"+r["rr"]+chr(10)+
            "Confianca:  "+str(r["score"])+"%"+chr(10)+chr(10)+
            "TIMEFRAMES"+chr(10)+
            "================================"+chr(10)+
            "Tendencia H1:    "+r["h1"]["trend"]+" (ADX="+str(r["h1"]["adx"])+")"+chr(10)+
            "Confirmacao M15: "+("OK" if r["m15"]["conf"] else "NOK")+" RSI="+str(r["m15"]["rsi"])+chr(10)+
            "Entrada M5:      Confirmada ("+r["tipo_vela"]+")"+chr(10)+chr(10)+
            "PONTUACAO"+chr(10)+
            "================================"+chr(10)+
            det_txt
        )
        resend.Emails.send({
            ...
            "to":email_destino,
            "subject":r["emoji"]+" XAUUSD "+r["trend"]+" @ "+str(r["preco"])+" | "+str(r["score"])+"% "+r["confianca"],
            "text":corpo
        })
    except: pass
...
if st.button("Analisar XAUUSD", type="primary"):
    with st.spinner("A analisar H1 + M15 + M5..."):
        r = analisar(ignorar_sessao=True)
        gc.collect()

    if r.get("sinal"):
        st.success(r["emoji"]+" SINAL CONFIRMADO — "+r["confianca"]+" ("+str(r["score"])+"%)")
        c1,c2,c3,c4 = st.columns(4)
        c1.metric("Direccao", r["trend"])
        c2.metric("Entrada", r["preco"])
        c3.metric("Confianca", str(r["score"])+"%")
        c4.metric("Classificacao", r["confianca"])
        c1b,c2b,c3b = st.columns(3)
        c1b.metric("Stop Loss", r["sl"])
        c2b.metric("Take Profit", r["tp"])
        c3b.metric("Risk Reward", r["rr"])

...
def detectar_estrutura(df, par):
    ...
    if len(sh)<2 or len(sl)<2:
        return {"estrutura":"INDEFINIDA","estado":"SEM DADOS","pontos":[],"bos":False,"choch":False,"ush":0,"usl":0}
    ...
    bos_bull=(p>ush)and(pp>ush); bos_bear=(p<usl)and(pp<usl)
...
def detectar_liquidez(df, par):
    ...
    for i in range(len(lows)):
        for j in range(i+1,len(lows)):
            ...
            if abs(v1-v2)/v1<0.0008:
                nivel=round((v1+v2)/2,p_round)
                eql=nivel; eql_estado="ROMPIDO" if p<nivel*0.999 else "ACTIVO"
                break
        if eql: break
    grab_topo=bsl_estado=="VARRIDA"
    grab_fundo=ssl_estado=="VARRIDA"
    bsl_varrida=bsl_estado in ["VARRIDA","INVALIDADA"]
    ssl_varrida=ssl_estado in ["VARRIDA","INVALIDADA"]
    return {"bsl":bsl,"bsl_estado":bsl_estado,"bsl_varrida":bsl_varrida,
            "ssl":ssl,"ssl_estado":ssl_estado,"ssl_varrida":ssl_varrida,
            "eqh":eqh,"eqh_estado":eqh_estado,"eql":eql,"eql_estado":eql_estado,
            "grab_topo":grab_topo,"grab_fundo":grab_fundo}

def detectar_order_blocks(df, par, tf_label):
    p_round=precisao(par)
    df=df.copy(); df["vf"]=df["corpo"]>df["media_corpo"]*1.4
    p=float(df["Close"].iloc[-1]); blocks=[]
    for tipo_ob in ["BULLISH","BEARISH"]:
        for i in range(len(df)-3,max(len(df)-40,0),-1):
            ...
            if tipo_ob=="BULLISH":
                ...
                cond_ativo=p>float(candle["High"]); invalido=p<float(candle["Low"])
            else:
                cond_ob=float(candle["Close"])>float(candle["Open"])
                cond_ativo=p<float(candle["Low"]); invalido=p>float(candle["High"])
            if cond_ob and cond_ativo and not invalido:
                alto=round(float(candle["High"]),p_round); baixo=round(float(candle["Low"]),p_round)
                pos=df.iloc[i+1:]; testes=int(((pos["Low"]<=alto)&(pos["High"]>=baixo)).sum())
                ...
                if forca>=70:
                    ...
                    blocks.append({"tipo":nome,"tf":tf_label,"alto":alto,"baixo":baixo,
                                   "hora_criacao":hora_criacao,"testes":testes,"forca":int(forca),"estado":"VALIDO"})
                break
    return blocks

def detectar_fvg(df, par, tf_label):
    p_round=precisao(par); p=float(df["Close"].iloc[-1]); fvgs=[]
    for direcao in ["BULL","BEAR"]:
        for i in range(len(df)-2,max(len(df)-30,1),-1):
            ...
            if direcao=="BULL" and float(df["Low"].iloc[i+1])>float(df["High"].iloc[i-1]):
                topo=round(float(df["Low"].iloc[i+1]),p_round); base=round(float(df["High"].iloc[i-1]),p_round)
...
def niveis_institucionais(par):
    ...
    try:
        ...
        pdh=round(float(dd["High"].iloc[-2]),p_round); pdl=round(float(dd["Low"].iloc[-2]),p_round)
        dw=obter_dados_smc(par,"1wk","10wk")
        pwh=round(float(dw["High"].iloc[-2]),p_round); pwl=round(float(dw["Low"].iloc[-2]),p_round)
...
def fluxo_institucional_h1(par):
    try:
        ...
        d=add_ind_smc(d); res=detectar_estrutura(d,par)
        return res["estrutura"]
    except: return "INDEFINIDO"

def calcular_qualidade(fluxo_h1, est_m15, est_m5, macro_alinhado):
    alinhados=sum([fluxo_h1==est_m15["estrutura"],est_m15["estrutura"]==est_m5["estrutura"],fluxo_h1==est_m5["estrutura"]])
...
def determinar_fase(est_m15, liq, obs, fvgs):
    ...
    elif grab and bos: return "MANIPULACAO","Varredura de liquidez seguida de rompimento estrutural. Continuacao provavel."
    elif grab and not bos and not choch: return "MANIPULACAO","Varredura detectada. Aguardar confirmacao: BOS ou CHoCH ainda nao ocorreu."
    elif liq_invalidada and bos: return "EXPANSAO","Liquidez consumida e rompimento confirmado. Movimento em curso."
    ...
    else: return "INDEFINIDA","Sem eventos estruturais relevantes no momento."
...
def escanear_par(par, macro):
    ...
    try:
        ...
        d15=add_ind_smc(d15)
        dh1=obter_dados_smc(par,"1h","10d"); dh1=add_ind_smc(dh1)
        dm5=obter_dados_smc(par,"5m","2d"); dm5=add_ind_smc(dm5)
        fluxo_h1=fluxo_institucional_h1(par)
        est_m15=detectar_estrutura(d15,par)
        est_m5=detectar_estrutura(dm5,par) if len(dm5)>30 else {"estrutura":"INDEFINIDA","estado":"SEM DADOS","pontos":[],"bos":False,"choch":False,"ush":0,"usl":0}
        ...
        todos_obs=ob_h1+ob_m15; todos_fvgs=fvg_h1+fvg_m15+fvg_m5
